...
# 3. 데이터 구조 설계 (합계 Row 추가)
# 이전 캐시와 충돌을 막기 위해 session_state 이름을 'df_v3'로 변경했습니다.
if "df_v3" not in st.session_state:
    ...
    for main, sub in departments:
        if main == "생산":
            ...
            initial_data.append({"대분류": main, "부서명": sub, "구분": "합계", "TO_인원": 0, "PO_인원": 0, "비고": ""}) # 합계 줄 추가
        else:
...
# (1) 각 부서의 '직접' + '간접'을 더해서 '합계' 줄에 자동으로 넣어줍니다.
...
# (2) 전체 총합 (합계 줄은 중복계산되므로 빼고 진짜 인원만 더합니다)
...
# 5. ⭐️ 핵심: 엑셀 셀 병합 시각적 효과 구현
df["대분류_표시"] = df["대분류"]
df["부서명_표시"] = df["부서명"]

for i in range(1, len(df)):
    if df.loc[i, "대분류"] == df.loc[i-1, "대분류"]:
        df.loc[i, "대분류_표시"] = ""  # 위와 같으면 글자를 지워서 병합된 것처럼 보임
    if df.loc[i, "부서명"] == df.loc[i-1, "부서명"]:
        df.loc[i, "부서명_표시"] = ""  # 위와 같으면 글자를 지워서 병합된 것처럼 보임
...
# 7. 사용자가 입력한 값 세션에 저장 (다음번 새로고침 시 합계 계산을 위해)
st.session_state.df_v3["TO_인원"] = edited_df["TO_인원"]
st.session_state.df_v3["PO_인원"] = edited_df["PO_인원"]
st.session_state.df_v3["비고"] = edited_df["비고"]
